...
class BoardWithUndo:
    def __init__(self, width, height, p1_sprites, p2_sprites):
        ...
        self.history = HistoryStack()  # Use the extended HistoryStack
...
class Board:
    def __init__(self,width,height, p1_sprites, p2_sprites):
        ...
        self.turn = 0
        self.history = Stack() # Initialize the history stack of the board
        self.history.push(self.get_board())  # Save the initial board state
    ...
    def undo_last_move(self, steps):
        for _ in range(steps):
            if not self.history.is_empty():
                self.board = self.history.pop()  # 從堆疊中彈出狀態
                self.turn -= 1  # 減少回合計數
            else:
                print("No more moves to undo!")  # 無更多歷史狀態
                break
...
# undo bottom
undo_button = pygame.Rect(900, 200, 200, 50)
...
while running:
    for event in pygame.event.get():
        ...
        if undo_button.collidepoint(x, y):
            if choice[current_player] == 0:  # 只有人類可以撤銷
                steps_to_undo = 1
                if choice[(current_player + 1) % 2] == 1:  # 如果對手是 AI
                    steps_to_undo = 2

                # 確保有足夠的歷史可以撤銷
                if len(board.history.history) >= steps_to_undo:
                    board.undo_last_move(steps_to_undo)

                    # 更新玩家邏輯
                    if steps_to_undo == 2:
                        current_player = (current_player - steps_to_undo) % 2
                    else:
